...
def tm_raw_to_detections(
    raw: list[dict[str, Any]], *, capability: str
) -> sv.Detections:
    """dicts Timonel crudos (label/score/bbox[+color]) → Detections con class_id NMS-A."""
    if not raw:
...
def detections_to_tm_raw(
    detections: sv.Detections, *, capability: str
) -> list[dict[str, Any]]:
    """Detections post-slicer → dicts Timonel sin track_id (coords hires)."""
    ...
    *,
    ...
    with httpx.Client(timeout=timeout) as client:
        ...
        def callback(image_slice: np.ndarray) -> sv.Detections:
            ...
            return tm_raw_to_detections(raw, capability=capability)
    ...
    if successes == 0 and failures > 0:
        ...
        return None
    return detections_to_tm_raw(merged, capability=capability)
