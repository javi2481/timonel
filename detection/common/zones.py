...
TIMONEL_ZONES_JSON = os.getenv("TIMONEL_ZONES_JSON", "").strip()
...
def parse_zones_json(raw: str) -> list[ZoneConfig]:
    """Parsea TIMONEL_ZONES_JSON; [] si vacío o inválido."""
    if not raw or not raw.strip():
        return []
    try:
        payload = json.loads(raw)
    except json.JSONDecodeError as exc:
        logger.warning("TIMONEL_ZONES_JSON invalid JSON: %s", exc)
        return []
    if not isinstance(payload, list):
        logger.warning("TIMONEL_ZONES_JSON must be a list of zone objects")
        return []
...
def load_zone_configs(raw: Optional[str] = None) -> list[ZoneConfig]:
    """Carga zonas desde ``raw`` o env ``TIMONEL_ZONES_JSON``."""
    return parse_zones_json(TIMONEL_ZONES_JSON if raw is None else raw)
