...
def _row_to_data_fields(det: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        if isinstance(det.get("polygon"), list)
        else None,
...
def tm_det_to_detections(det: dict[str, Any]) -> sv.Detections:
...
def detections_to_tm_dets(detections: sv.Detections) -> list[dict[str, Any]]:
    """Survivors post-NMS → dicts Timonel (extras del survivor vía ``tm_row``)."""
    out: list[dict[str, Any]] = []
    n = len(detections)
    rows = detections.data.get("tm_row") if detections.data else None
    for i in range(n):
...
def apply_cross_cap_nms(
    ...
    *,
    ...
    pieces = [tm_det_to_detections(d) for d in with_bbox]
    ...
    survivors = detections_to_tm_dets(kept)
