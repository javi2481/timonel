...
from detection.common.tiled_infer import (
    ...
    detections_to_tm_raw,
    infer_tiled_sync,
    tm_raw_to_detections,
)
...
class TmDetectionsRoundTripTests(unittest.TestCase):
    def test_round_trip_preserves_bbox_label_color(self) -> None:
        ...
        dets = tm_raw_to_detections(raw, capability="vehicles")
        self.assertEqual(len(dets), 1)
        self.assertIsNotNone(dets.class_id)
        self.assertIsNotNone(dets.confidence)
        back = detections_to_tm_raw(dets, capability="vehicles")
...
class EmptyDetectionsTests(unittest.TestCase):
    def test_empty_tm_raw(self) -> None:
        dets = tm_raw_to_detections([], capability="objects")
        self.assertEqual(len(dets), 0)
        self.assertEqual(detections_to_tm_raw(dets, capability="objects"), [])
