...
def _cfg(**kwargs) -> LifecycleConfig:
    base = dict(
        ...
        cap_containers={
            "pedestrians": "vi-paddlex-pedestrians",
            "face_id": "vi-paddlex-face-id",
        },
    )
    base.update(kwargs)
    ...
    def test_custom_pause_caps(self) -> None:
        with patch.dict(
            os.environ,
            {
                ...
                "LIFECYCLE_PAUSE_CAPS": "pedestrians,face_id=vi-custom-fid",
        ...
        self.assertEqual(cfg.cap_containers["pedestrians"], "vi-paddlex-pedestrians")
        self.assertEqual(cfg.cap_containers["face_id"], "vi-custom-fid")
...
class TestContainerLifecycle(unittest.IsolatedAsyncioTestCase):
    async def test_ensure_awake_unpauses(self) -> None:
        engine = FakeDocker(
            {
                "vi-paddlex-pedestrians": "paused",
                "vi-paddlex-face-id": "running",
            }
        )
        ...
        self.assertEqual(engine.unpaused, ["vi-paddlex-pedestrians"])
        self.assertEqual(engine.states["vi-paddlex-pedestrians"], "running")

    async def test_pause_idle_respects_touch(self) -> None:
        engine = FakeDocker(
            {
                "vi-paddlex-pedestrians": "running",
                "vi-paddlex-face-id": "running",
            }
        )
        ...
        self.assertEqual(engine.paused, ["vi-paddlex-face-id"])
    ...
    async def test_disabled_noop(self) -> None:
        engine = FakeDocker({"vi-paddlex-pedestrians": "running"})
