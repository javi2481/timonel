...
NO importa adapter.timonel (imagen liviana): define su propio modelo
...
def _enforce_production_secrets() -> None:
    ...
    if not key or key == "demo":
        raise SystemExit(
            "TIMONEL_ENV=production requires RULES_SINK_API_KEY set to a non-demo secret"
        )
...
class VehiclePayloadIn(BaseModel):
    """Subconjunto permisivo de timonel.VehiclePayload — solo lo que la regla lee."""
...
class PerceptionEventIn(BaseModel):
    """
    Modelo local del PerceptionEvent de timonel (schema 1.0).
    ...
    contrato completo ni importar timonel.
    """
...
app = FastAPI(
    title="Timonel Rules Sink",
    description="Capa de reglas headless — evalúa PerceptionEvents reenviados por el adaptador (contrato Timonel)",
    version="0.1.0",
)
