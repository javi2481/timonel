...
#
# VI patch: PaddleX 3.7.2 FaceFeaturePredictor returns feature shape (1, D).
# np.array([feat, ...]) → (N, 1, D); FaissBuilder.build then unpacks
# `vector_num, vector_dim = features.shape` and raises ValueError. Flatten to
# (N, D) before index build / append (face_id gallery).
...
def _features_as_2d(features: np.ndarray) -> np.ndarray:
    """Ensure gallery features are (N, D) for faiss index_factory."""
    if features.ndim > 2:
...
class FaissBuilder:
    ...
    def build(
        ...
        features = _features_as_2d(np.array(features).astype(dtype))
