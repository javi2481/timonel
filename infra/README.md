...
`paddlex*` usan la misma build y se diferencian por `TIMONEL_PIPELINE` / `TIMONEL_PORT`.

## Cómo funciona

1. `Dockerfile.paddlex` instala `paddlex[cv,serving,ocr]` **pineado a `==3.7.2`**
   (sin fallbacks `||`): las dos builds (`timonel/paddlex:local`
...
4. `TIMONEL_USE_HPIP=1` añade `--use_hpip` (High Performance Inference).
...
| Env | Efecto |
|-----|--------|
| `TIMONEL_USE_HPIP=0` (default) | Serve estándar |
| `TIMONEL_USE_HPIP=1` | `--use_hpip` en entrypoint |
...
El entrypoint exige `nvidia-smi` usable cuando `TIMONEL_DEVICE=gpu`; si no hay GPU/runtime, el contenedor falla al arrancar (en vez de quedar “up” sin servir).
