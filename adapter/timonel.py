"""
Contrato Timonel (schema_version=\"1.0\").

Ubicación: adapter/timonel.py — contrato portable del producto.
...
logger = logging.getLogger("timonel")
...
class TextPayload(BaseModel):
    ...
    # Polígono OCR original (dt_polys); aditivo SCHEMA 1.0.
...
def _normalize_detection(raw: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "polygon": (
            raw.get("polygon")
            if isinstance(raw.get("polygon"), list) and len(raw.get("polygon") or []) >= 3
            else None
        ),
        "zones": (
...
def _consolidate_text(cls, track_id, detections, observed_at, location):
    ...
    poly = best.get("polygon") if isinstance(best.get("polygon"), list) else None
