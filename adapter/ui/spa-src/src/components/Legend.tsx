...
export function Legend({ events, visibility }: Props) {
  ...
  return (
    <div className="tm-legend-card">
      <div className="tm-legend-h">
        Leyenda <span className="tm-legend-hint">solo lo visible</span>
      </div>
      <div className="tm-legend">
        {items.map((it) => (
          <span className="tm-legend-it" key={it.key}>
            <span className="tm-legend-sw" style={{ background: it.color }} />
            {it.label}
          </span>
        ))}
      </div>
