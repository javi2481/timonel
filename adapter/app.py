...
from adapter.timonel import PerceptionEvent
...
TIMONEL_ENV = os.getenv("TIMONEL_ENV", "development").strip().lower()


def _enforce_production_secrets() -> None:
    """En TIMONEL_ENV=production exige JETLINKS_API_KEY fuerte (!= demo / vacío)."""
    if TIMONEL_ENV != "production":
        return
    key = (JETLINKS_API_KEY or "").strip()
    if not key or key == "demo":
        raise SystemExit(
            "TIMONEL_ENV=production requires JETLINKS_API_KEY set to a non-demo secret"
        )
...
def _flush_track(track_id: str) -> list[PerceptionEvent]:
    """Saca un track del caché y pide a timonel que construya PerceptionEvent(s).

    No confundir con `timonel._emit_track`, que arma el payload por entity_type.
    """
    st = state()
...
async def _media_watch_loop(stop: asyncio.Event) -> None:
    """Detecta JPGs nuevos en MEDIA_DIR/images y los auto-selecciona.

    Bootstrap: solo marca mtimes vistos — la SPA arranca en idle (sin foto)
    hasta upload/select explícito o un archivo nuevo/actualizado en la carpeta.
    Tras `POST /media/clear` tampoco re-selecciona lo ya visto.
    """
    while not stop.is_set():
        try:
            st = state()
            ...
            if not st.media_watch_bootstrapped:
                ...
                logger.info(
                    "Media watch bootstrap — idle (%d archivos vistos, sin auto-select)",
                    len(current),
                )
            else:
...
async def lifespan(_app: FastAPI):
    ...
    logger.info(
        "Adapter started TTL=%.1fs buffer=%d TIMONEL_ENV=%s media_watch=%.1fs",
        TRACK_TTL_SECONDS,
        EVENTS_BUFFER_SIZE,
        TIMONEL_ENV,
...
app = FastAPI(
    title="Timonel Adapter",
    description="Normaliza PaddleX → timonel (schema 1.0)",
    version="0.1.0",
    lifespan=lifespan,
)
