# Smoke PR3: zonas runtime (`VI_ZONES_JSON` → epp → rules)
...
2. Compose pasa `VI_ZONES_JSON` al bridge (ver `docker-compose.yml`).
3. Perfil rules + webhook:

```bash
# .env (ejemplo smoke — polígono full-frame)
VI_ZONES_JSON=[{"id":"no_parking","polygon":[[0.0,0.0],[1.0,0.0],[1.0,1.0],[0.0,1.0]]}]
...
```bash
docker inspect vi-rules-sink --format "{{.Config.Cmd}}"
