...
Cierra el gap que el harness no ve (`merge_coco`, timonel, preview).
...
| Nota ops | Antes de la corrida válida: host→`:8081`/`:8082` daba `RemoteDisconnected` (vehicles `:8080` OK). `docker restart tm-paddlex-ocr tm-paddlex-objects` restauró docs/predict; la 1ª corrida direct con objects/OCR a 0.0 se descarta |
